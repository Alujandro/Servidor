...
function comprobar_usuario($nombre, $clave){
    try{
        $db=new PDO(CADENA_CONEXION, USUARIO_CONEXION, CLAVE_CONEXION);
...
function cargar_productos($codigosProductos){
    try{
        ...
        $ins="SELECT * FROM productso WHERE CodProd in ($texto_in)";
        $resul=$bd->query($ins);
        if (!$resul){
            return false;
        }
...
function insertar_pedido($carrito, $codRes){
    try{
        ...
        $sql="INSET into pedidos(Fecha, Enviado, Restaurante) VALUES ('$hora',0, $codRes)";
        $resul=$bd->query($sql);
        if (!$resul){
            return false;
        }
        $pedido=$bd->lastInsertId();
        foreach ($carrito as $codProd=>$uniddades){
