...
//Añadido por mi, para enviar todos los pedidos
function cargar_pedidos($resta){
    try{
        $bd = new PDO(CADENA_CONEXION, USUARIO_CONEXION, CLAVE_CONEXION);
        $ins = "SELECT pedidos.CodPed, pedidos.Fecha, pedidos.Enviado, productos.nombre, productos.descripcion, pedidosproductos.unidades FROM productos, pedidosproductos, pedidos  WHERE pedidosproductos.CodProd=productos.CodProd AND pedidos.Restaurante=$resta AND pedidos.CodPed=pedidosproductos.CodPed";
        $resul = $bd->query($ins);
        if(!$resul){
            return FALSE;
        }
